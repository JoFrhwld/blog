...
As I scroll through my feeds, I often come across a really cool looking package, or feature of a new package, that I think looks really cool, and then I forget to back to really kick the tires and see how it works. So I've decided to try to set up a workflow where I send the docs or `{pkgdown}` pages for the package to a Trello board, and then come back maybe once a month and experiment with them in a blog post.
...
Default is *ok*, bu for this data set needs to be a bit less jagged, which can be adjusted with the `concavity` argument. I'll also throw in a fill color.
...
I kind of wish this was separated out into a few different, more conventional, ggplot2 layers. It's called a `geom_` but the convex hulls are definitely `stat_`s. But the convex hull statistic layer isn't exposed to users, so you can't mix-and-match convex hull estimation and the geom used to draw it. On the other hand, I can see that it's much more souped up than a typical geom. For example, you can filter the data within the `aes()` mapping.
